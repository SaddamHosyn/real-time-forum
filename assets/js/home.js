...
function setupCreatePostForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const checkedTopics = form.querySelectorAll('input[name="create-post-topic"]:checked');
    ...
    if (errors.length > 0) {
      ...
      console.log("Form submitted successfully!", { title, content, topics: Array.from(checkedTopics).map(cb => cb.value) });
      currentModal.remove();
      currentModal = null;
    }
  });
